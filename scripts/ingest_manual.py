...
def main():
    ...
    log.info("📥 Loading PDF text + extracting images...")
    ...
    log.info("🧠 Embedding + upserting text chunks...")
    ...
    log.info(f"✅ Upserted {len(ids)} text chunks.")

    # 4b) Upsert images (CLIP) + captions also into text index
    log.info("🧩 Embedding + upserting images (+captions)...")
    ...
    if all_items:
        ...
        log.info(f"✅ Upserted {len(img_ids)} images and {len(cap_ids)} captions.")
    else:
        log.info("No images found to embed.")

    log.info("🎉 Manual ingestion finished.")
